...
def blip_finetune_fiq(train_dress_types: List[str], val_dress_types: List[str],
    ...
    training_path: Path = Path(
        base_path / f"models/blip_finetuned_on_fiq_{blip_model_name}_{training_start}")
    ...
    model, vis_processors, txt_processors = load_model_and_preprocess(name=blip_model_name, model_type="pretrain", is_eval=False, device=device)
    # model.eval()
    model.eval().float()
        ...
        with experiment.train():
            ...
            for idx, (reference_images, target_images, captions) in enumerate(train_bar):
                ...
                with torch.cuda.amp.autocast():
                    reference_features = model.extract_features({"image":reference_images}, mode="image").image_embeds
                    target_features = model.extract_features({"image":target_images}, mode="image").image_embeds
                    caption_features = model.extract_features({"text_input":text_inputs}, mode="text").text_embeds   

                    predicted_features = torch.stack( [combining_function(reference_features[:,i,:], caption_features[:,0,:]) for i in range(reference_features.shape[1])],dim=1)

                    logits = [100 * predicted_features[:,i,:] @ F.normalize(target_features[:,i,:],dim=-1).T for i in range(reference_features.shape[1])]
        ...
        if epoch % validation_frequency == 0:
            with experiment.validate():
                ...
                for relative_val_dataset, classic_val_dataset, idx in zip(relative_val_datasets, classic_val_datasets,
                    ...
                    if encoder == 'text':
                        ...
                        index_features, index_names = extract_index_features_blip(classic_val_dataset, model)
                    recall_at10, recall_at50 = compute_fiq_val_metrics_blip(relative_val_dataset, model, index_features,
                                                                       index_names, combining_function,txt_processors["eval"])    
            ...
            if save_training:
                if save_best and results_dict['average_recall'] > best_avg_recall:
                    ...
                    save_model('tuned_blip_best', epoch, model, training_path)
                elif not save_best:
                    save_model(f'tuned_blip_{epoch}', epoch, model, training_path)


def blip_finetune_cirr(num_epochs: int, blip_model_name: str, learning_rate: float, batch_size: int,
                       validation_frequency: int, transform: str, save_training: bool, encoder: str, save_best: bool,
                       **kwargs):
    """
    ...
    training_start = datetime.now().strftime("%Y-%m-%d_%H:%M:%S")
    training_path: Path = Path(
        base_path / f"models/blip_finetuned_on_cirr_{blip_model_name}_{training_start}")
    training_path.mkdir(exist_ok=False, parents=True)
            ...
            for idx, (reference_images, target_images, captions) in enumerate(train_bar):
                ...
                with torch.cuda.amp.autocast():
                    with torch.autograd.detect_anomaly():
                        ...
                        assert not torch.any(torch.isnan(reference_features))
                        assert not torch.any(torch.isnan(target_features))
                        assert not torch.any(torch.isnan(text_features))
            ...
            with experiment.validate():
                ...
                if save_training:
                    if save_best and results_dict['arithmetic_mean'] > best_arithmetic:
                        ...
                        save_model('tuned_blip_arithmetic', epoch, blip_model, training_path)
                    if save_best and results_dict['harmonic_mean'] > best_harmonic:
                        best_harmonic = results_dict['harmonic_mean']
                        save_model('tuned_blip_harmonic', epoch, blip_model, training_path)
                    if save_best and results_dict['geometric_mean'] > best_geometric:
                        best_geometric = results_dict['geometric_mean']
                        save_model('tuned_blip_geometric', epoch, blip_model, training_path)
                    if not save_best:
                        save_model(f'tuned_blip_{epoch}', epoch, blip_model, training_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--blip-model-name", default="RN50x4", type=str, help="blip model to use, e.g 'RN50', 'RN50x4'")
    parser.add_argument("--encoder", default='both', type=str,
                        help="Which blip encoder to fine-tune, should be in ['both', 'text', 'image']")

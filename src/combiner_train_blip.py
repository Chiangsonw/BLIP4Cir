...
def combiner_training_fiq(train_dress_types: List[str], val_dress_types: List[str],
                          projection_dim: int, hidden_dim: int, num_epochs: int, model_name: str,
                          combiner_lr: float, batch_size: int, bs: int, validation_frequency: int,
    ...
    :param model_name: CLIP model you want to use: "RN50", "RN101", "RN50x4"...
    :param combiner_lr: Combiner learning rate
    :param batch_size: batch size of the Combiner training
    :param bs: batch size of the CLIP feature extraction
    ...
    :param kwargs: if you use the `targetpad` transform you should prove `target_ratio` as kwarg. If you want to load a
                fine-tuned version of clip you should provide `model_path` as kwarg.
    """

    training_start = datetime.now().strftime("%Y-%m-%d_%H:%M:%S")
    training_path: Path = Path(
        base_path / f"models/combiner_trained_on_fiq_{model_name}_{training_start}")
    training_path.mkdir(exist_ok=False, parents=True)

    # Save all the hyperparameters on a file
    with open(training_path / "training_hyperparameters.json", 'w+') as file:
        json.dump(training_hyper_params, file, sort_keys=True, indent=4)
    model, vis_processors, txt_processors = load_model_and_preprocess(name=model_name, model_type="pretrain", is_eval=True,device = device)
    # model = nn.DataParallel(model)
    model = model.to(device)

    model.eval()
    # model = model.float().to(device, non_blocking=True)
    model = model.float()
    ...
    for idx, dress_type in enumerate(val_dress_types):
        ...
        relative_val_dataset = FashionIQDataset('val', [dress_type], 'relative', vis_processors["eval"])
        relative_val_datasets.append(relative_val_dataset)
        classic_val_dataset = FashionIQDataset('val', [dress_type], 'classic', vis_processors["eval"])
        index_features_and_names = extract_index_features_blip(classic_val_dataset, model)
    ...
    for epoch in range(num_epochs):
        ...
        #     clip.model.convert_weights(model)  # Convert CLIP model in fp16 to reduce computation and memory
        with experiment.train():
            ...
            for idx, (reference_images, target_images, captions) in enumerate(train_bar):  # Load a batch of triplets
                ...
                input_captions = generate_randomized_fiq_caption_blip(flattened_captions,txt_processors["eval"])
                ...
                with torch.no_grad():
                    reference_images_list = torch.split(reference_images, bs)
                    reference_image_features = torch.vstack(
                        [model.extract_features({"image":mini_batch}, mode="image").image_embeds    for mini_batch in reference_images_list])
                    target_images_list = torch.split(target_images, bs)
                    target_image_features = torch.vstack(
                        [model.extract_features({"image":mini_batch}, mode="image").image_embeds    for mini_batch in target_images_list])
                ...
                with torch.cuda.amp.autocast():
                    logits = [combiner(reference_image_features[:,i,:], text_features[:,0,:], target_image_features[:,i,:]) for i in range(reference_image_features.shape[1])]
                    ground_truth = torch.arange(images_in_batch, dtype=torch.long, device=device)
                    loss = 0
                    for i in range(reference_image_features.shape[1]):
                        loss+=crossentropy_criterion(logits[i], ground_truth)/reference_image_features.shape[1]
        ...
        if epoch % validation_frequency == 0:
            ...
            with experiment.validate():
                ...
                for relative_val_dataset, index_features, index_names, idx in zip(relative_val_datasets,
                                                                                  ...
                                                                                  index_names_list,
                                                                                  idx_to_dress_mapping):
                    recall_at10, recall_at50 = compute_fiq_val_metrics_blip(relative_val_dataset, model, index_features,
                                                                       index_names, combiner.combine_features,txt_processors["eval"])
...
def combiner_training_cirr(projection_dim: int, hidden_dim: int, num_epochs: int, model_name: str,
                           combiner_lr: float, batch_size: int, bs: int, validation_frequency: int, 
                           save_training: bool, save_best: bool, **kwargs):
    """
    ...
    training_start = datetime.now().strftime("%Y-%m-%d_%H:%M:%S")
    training_path: Path = Path(
        base_path / f"models/combiner_trained_on_cirr_{model_name}_{training_start}")
    training_path.mkdir(exist_ok=False, parents=True)
    ...
    val_index_features, val_index_names = extract_index_features_blip(classic_val_dataset, model)
    ...
    for epoch in range(num_epochs):
        ...
        with experiment.train():
            ...
            for idx, (reference_images, target_images, captions) in enumerate(train_bar):  # Load a batch of triplets
                ...
                text_inputs = [txt_processors["eval"](i) for i in captions]
                ...
                # Compute the logits and the loss
                with torch.cuda.amp.autocast():
                    logits = [combiner(reference_features[:,i,:], text_features[:,0,:], target_features[:,i,:]) for i in range(reference_features.shape[1])]
                    ground_truth = torch.arange(images_in_batch, dtype=torch.long, device=device)
                    loss = 0
                    for i in range(reference_features.shape[1]):
                        loss+=crossentropy_criterion(logits[i], ground_truth)/reference_features.shape[1]
        ...
        if epoch % validation_frequency == 0:
            ...
            with experiment.validate():
                ...
                results = compute_cirr_val_metrics_blip(relative_val_dataset, model, val_index_features,
                                                   val_index_names, combiner.combine_features, txt_processors["eval"])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model-name", default="RN50x4", type=str, help="CLIP model to use, e.g 'RN50', 'RN50x4'")
